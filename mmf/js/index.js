var CONTRACT_ADDRESS = '0x8f6396dA39ED8d010c7C637e0978327258729e72';

var referrer = '0x18ED1192fc3aec92EBd77E4cCEb740F6D747a617'
var upline = '0x18ED1192fc3aec92EBd77E4cCEb740F6D747a617'
...
var currentAddr;
var contract = null;
var gasPrice = '90000000000' //90000000000 MATIC GAS
...
async function Connect() {
    ...
    //setTimeout(checkForPolygon, 1500)
}
async function checkForPolygon() {
    try {
        await window.Polygon.enable()
        console.log(typeof(window.Polygon))
        if (window.Polygon) {
            console.log('Polygon')
            await Polygon.enable()
            window.web3 = new Web3(window.Polygon)
            let accounts = await web3.eth.getAccounts()
            currentAddr = accounts[0]
...
function stakematic() {
if (contract) {
	var amount = document.getElementById("app__inputmatic").value;
...
function getContractBalance() {
	contract.methods.getBalance().call().then(res=>{
        ...
        $("#contractBalance").text(res + " MATIC");
        console.log(res);
    })
